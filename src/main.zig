...
//   wl    -> core Wayland (wl_compositor, wl_seat, …)
...
pub fn main() !void {
    ...
    // The state socket a panel reads for desktops/focus/titles. reach draws no
    // bar itself, so this is the only way any of that leaves the window manager.
    ipc.start();
    defer ipc.stop();
